...
namespace PokerTracker2.Windows
{
    public partial class LoginWindow : Window
    {
        ...
        private async void TestFirebase_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                LoggingService.Instance.Info("TestFirebase_Click called", "LoginWindow");
                ...
                if (!connectionTest)
                {
                    LoggingService.Instance.Error("Firebase connection test failed!", "LoginWindow");
                    ...
                    StatusText.Text = "Status: Firebase test failed";
                    return;
                }
                ...
                if (!writeTest)
                {
                    LoggingService.Instance.Error("Firebase write test failed!", "LoginWindow");
                    MessageBox.Show("❌ Firebase write test failed!\n\nCheck the debug output for details.", 
                        "Firebase Test", MessageBoxButton.OK, MessageBoxImage.Error);
                    StatusText.Text = "Status: Firebase write test failed";
                    return;
                }
                
                // Get database structure
                var dbStructure = await GetDatabaseStructureAsync(firebaseService);
                
                var message = $"✅ Firebase connection test successful!\n\n" +
                             $"📡 Connection: Working\n" +
                             $"✍️ Write Operations: Working\n" +
                             $"📊 Database Structure:\n{dbStructure}";
                
                MessageBox.Show(message, "Firebase Test", MessageBoxButton.OK, MessageBoxImage.Information);
                StatusText.Text = "Status: Firebase test successful";
            }
            catch (Exception ex)
            {
                LoggingService.Instance.Error($"❌ Firebase test failed with exception:\n\n{ex.Message}", "LoginWindow", ex);
        ...
        private async Task<string> GetDatabaseStructureAsync(FirebaseService firebaseService)
        {
            try
            {
                ...
                try
                {
                    ...
                    structure.AppendLine($"📁 player_profiles collection:");
                    structure.AppendLine($"   • Found {profiles.Count} profiles");
                    if (profiles.Count > 0)
                    {
                ...
                try
                {
                    ...
                    structure.AppendLine($"📁 sessions collection:");
                    structure.AppendLine($"   • Found {sessions.Count} sessions in last year");
                    if (sessions.Count > 0)
                    {
                ...
                // Test test collection (for connection verification)
                try
                {
                    var testCollection = await firebaseService.TestConnectionAsync();
                    structure.AppendLine($"📁 test collection:");
                    structure.AppendLine($"   • Connection: {(testCollection ? "✅ Working" : "❌ Failed")}");
                    structure.AppendLine();
                }
                catch (Exception ex)
                {
                    structure.AppendLine($"❌ test collection error: {ex.Message}");
                }
